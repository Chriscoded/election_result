...
class HomeController extends Controller
{
    ...
    {
        // $results = Polling_Unit::with('pu_results');
    ...
    public function lga_result($id)
    {
        ...
        ->join('announced_pu_results', function($join){
          ...
          })->where('polling_unit.lga_id', '=', $id)->get();
        ...
        $lga = \DB::table('lga')->where('uniqueid', '=', $id)->get();
        
        

          //dd($pdp,$results);
        //   $balance = DB::table('data')->where('user_id' '=' $id)->sum('balance');
        return view('Home.lga_result', compact(['PDP','DPP','ACN','PPA','CDC','JP','ANPP', 'lga']));
